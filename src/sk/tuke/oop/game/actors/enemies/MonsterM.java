 ...
 */
public class MonsterM extends AbstractCharacter implements Movable, Enemy {

    private Rotation rot;
    
    public MonsterM(String name) {
        super(name);
        
        rot = Rotation.UP;
        setEnergy(20);
        
        setAnimation(new Animation("resources/sprites/warrior_alien_lv2.png", 32, 32, 100));
    }
            ...
            if(act instanceof Hero) {
                if(abs(act.getX() - this.getX()) < 50 && abs(act.getY() - this.getY()) < 100) {
                    ...
                    if(this.intersects(act)) {
                        ((Hero)act).setEnergy(((Hero)act).getEnergy() - 2);
                    }
                }
            }
        }  
